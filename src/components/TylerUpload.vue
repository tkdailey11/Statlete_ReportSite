...
export default {
  ...
  methods: {
      ...
      submit(){
        ...
        if(confirm("Do you want to sumit the following? " + JSON.stringify(data))){
            firebase.database().ref('/Tyler/entries').update({
                [week] : entries
            }).then(function(val){
      ...
      selectionChanged() {
            ...
            var self = this;
            firebase.database().ref('/Tyler/entries').child(week).once('value', function(snapshot){
  ...
  created() {
    var self = this;
    firebase.database().ref('/Tyler/entries/Week 01').once('value', function(snapshot){
        if(snapshot.val() == null){
            self.entriesList = []
        }
        else{
