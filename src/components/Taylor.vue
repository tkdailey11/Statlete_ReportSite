...
export default {
  ...
  methods: {
    ...
    downloadFile(file) {
      ...
      storageRef.child('/Taylor/' + file.name).getDownloadURL().then(function(url) {
        axios({
          url: url,
          method: 'GET',
          responseType: 'blob'
          ...
          link.href = url;
          link.setAttribute('download', 'TaylorDejonge-' + file.name);
  ...
  created() {
    ...
    firebase.database().ref('/Taylor').once('value').then(function(snapshot){
      ...
      filenameArr.forEach(function(element, index){
        var taylorRef = storageRef.child('Taylor').child(element);

        taylorRef.getMetadata().then(function(metadata) {
          taylorRef.getDownloadURL().then(function(url) {
