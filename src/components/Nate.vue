...
export default {
  ...
  methods: {
    ...
    downloadFile(file) {
      ...
      storageRef.child('/Nate/' + file.name).getDownloadURL().then(function(url) {
        axios({
          url: url,
          method: 'GET',
          responseType: 'blob'
          ...
          link.href = url;
          link.setAttribute('download', 'NateGygi-' + file.name);
  ...
  created() {
    ...
    firebase.database().ref('/Nate').once('value').then(function(snapshot){
      ...
      filenameArr.forEach(function(element, index){
        var nateRef = storageRef.child('Nate').child(element);

        nateRef.getMetadata().then(function(metadata) {
          nateRef.getDownloadURL().then(function(url) {
